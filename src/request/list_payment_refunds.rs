...
#[derive(Clone)]
pub struct ListPaymentRefundsRequest<'a> {
...
impl<'a> ListPaymentRefundsRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<ListPaymentRefundsResponse> {
        let mut r = self.http_client.client.get("/v2/refunds");
        ...
        if let Some(ref unwrapped) = self.source_type {
            r = r.query("source_type", &unwrapped.to_string());
        }
    ...
    pub fn source_type(mut self, source_type: &str) -> Self {
        self.source_type = Some(source_type.to_owned());
        self
    }
...
impl<'a> ::std::future::IntoFuture for ListPaymentRefundsRequest<'a> {
    type Output = httpclient::InMemoryResult<ListPaymentRefundsResponse>;
