...
#[derive(Clone)]
pub struct DeprecatedBatchRetrieveInventoryCountsRequest<'a> {
...
impl<'a> DeprecatedBatchRetrieveInventoryCountsRequest<'a> {
    pub async fn send(
        self,
    ) -> ::httpclient::InMemoryResult<BatchRetrieveInventoryCountsResponse> {
        let mut r = self.http_client.client.post("/v2/inventory/batch-retrieve-counts");
...
for DeprecatedBatchRetrieveInventoryCountsRequest<'a> {
