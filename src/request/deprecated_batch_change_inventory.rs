...
#[derive(Clone)]
pub struct DeprecatedBatchChangeInventoryRequest<'a> {
    pub(crate) http_client: &'a SquareApiClient,
    pub changes: Option<Vec<InventoryChange>>,
    pub idempotency_key: String,
    pub ignore_unchanged_counts: Option<bool>,
}
impl<'a> DeprecatedBatchChangeInventoryRequest<'a> {
    pub async fn send(
        self,
    ) -> ::httpclient::InMemoryResult<BatchChangeInventoryResponse> {
        let mut r = self.http_client.client.post("/v2/inventory/batch-change");
...
impl<'a> ::std::future::IntoFuture for DeprecatedBatchChangeInventoryRequest<'a> {
