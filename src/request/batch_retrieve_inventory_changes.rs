...
#[derive(Clone)]
pub struct BatchRetrieveInventoryChangesRequest<'a> {
...
impl<'a> BatchRetrieveInventoryChangesRequest<'a> {
    pub async fn send(
        self,
    ) -> ::httpclient::InMemoryResult<BatchRetrieveInventoryChangesResponse> {
        let mut r = self.http_client.client.post("/v2/inventory/changes/batch-retrieve");
...
impl<'a> ::std::future::IntoFuture for BatchRetrieveInventoryChangesRequest<'a> {
