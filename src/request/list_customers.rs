...
impl<'a> ListCustomersRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<ListCustomersResponse> {
        let mut r = self.http_client.client.get("/v2/customers");
        ...
        if let Some(ref unwrapped) = self.sort_field {
            r = r.query("sort_field", &unwrapped.to_string());
        }
    ...
    pub fn sort_field(mut self, sort_field: &str) -> Self {
        self.sort_field = Some(sort_field.to_owned());
        self
    }
...
impl<'a> ::std::future::IntoFuture for ListCustomersRequest<'a> {
    type Output = httpclient::InMemoryResult<ListCustomersResponse>;
